...
export default class GamePage {
  ...
  bindTouchEvent() {
    ...
    wx.onTouchEnd(() => {
      ...
      this.bottle.jump(pressTime, currentY, nextY, () => { this.checkCollision() })
    })
  }
  ...
  checkCollision() {
    ...
    if (distanceToNext < CENTER_RADIUS) {
      ...
      if (nextBlock.isHard) { scoreText3d.showScore(this.scene.instance, baseScore, nextBlock.instance.position) }
      particle.createDust(this.scene.instance, this.bottle.obj.position)
      this.successJump(nextBlock)
      ...
      if (distanceToNext >= NEXT_RADIUS && distanceToNext < NEXT_RADIUS + 1.5) {
        if (this.bottle.direction === 'x') { fallType = dx < 0 ? 'tiltBackward' : 'tiltForward' } 
        else { fallType = dz > 0 ? 'tiltBackward' : 'tiltForward' }
  ...
  startBonusTimer(block) {
    ...
    let bonusScore = 0
    let audioName = ''
    if (block.skin === 'store') { bonusScore = 15; audioName = 'store' } 
    else if (block.skin.includes('disk')) { bonusScore = 30; audioName = 'sing' }
    if (bonusScore > 0) {
